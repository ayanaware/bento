	...
	public emit(name: string, ...args: Array<any>): void {
		...
		for (const handler of handlers) {
			try {
				...
				if (name === 'error') throw new LiteEmitterError('Caught Error in "error" handler function').setCause(e as Error);

				// emit as an error
				this.emit('error', new LiteEmitterError('Caught Error in handler function').setCause(e as Error));
			}
		}
	}
}
